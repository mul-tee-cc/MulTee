...
if __name__ == '__main__':
    ...
    usage_mask_pu = Attribute.AttributeName('Cryptographic Usage Mask')
    value = CryptographicUsageMask(UsageMaskEnum.VERIFY.value)
    usage_mask_pu = Attribute(attribute_name=usage_mask_pu, attribute_value=value)

    usage_mask_pr = Attribute.AttributeName('Cryptographic Usage Mask')
    value = CryptographicUsageMask(UsageMaskEnum.SIGN.value)
    usage_mask_pr = Attribute(attribute_name=usage_mask_pr, attribute_value=value)

    attribute_type = AttributeType.CRYPTOGRAPHIC_LENGTH
    length_obj = attribute_factory.create_attribute(attribute_type, length)

    attributes_pu = [algorithm_obj, length_obj, name, usage_mask_pu]
    attributes_pr = [algorithm_obj, length_obj, name, usage_mask_pr]
    ...
    if opts.operation_policy_name is not None:
        ...
        attributes_pu.append(opn)
        attributes_pr.append(opn)
    ...
    private = TemplateAttribute(
        attributes=attributes_pr,
        tag=enums.Tags.PRIVATE_KEY_TEMPLATE_ATTRIBUTE
    )
    public = TemplateAttribute(
        attributes=attributes_pu,
